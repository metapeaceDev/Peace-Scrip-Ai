...
const LoRASetup: React.FC<LoRASetupProps> = ({ onComplete, onSkip }) => {
  ...
  useEffect(() => {
    ...
    const interval = autoCheckEnabled 
          ...
          <div className="flex items-center gap-3">
            {checking ? (
              ...
              <>
                ...
                <div>
                  <div className="text-green-400 font-semibold">✅ All Required LoRA Models Installed!</div>
              ...
              <>
                ...
                <div>
                  <div className="text-amber-400 font-semibold">⚠️ Missing {missingModels.length} Required Model{missingModels.length > 1 ? 's' : ''}</div>
                  <div className="text-amber-300/70 text-sm">Please install to enable Face ID generation</div>
                </div>
              </>
            )}
          </div>
                    ...
                    <div key={model.name} className={`border rounded-lg p-4 ${
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-500">
                            File: <code className="bg-gray-900 px-2 py-1 rounded">{model.filename}</code>
                          </div>
                        </div>
                        <button
                      ...
                      {isExpanded && !installed && (
                        <div className="mt-4 pt-4 border-t border-gray-700">
                          <h5 className="font-semibold text-white mb-2">📝 Installation Instructions:</h5>
...
{instructions.steps.join('\n')}
            ...
            {optionalModels.length > 0 && (
              <div className="mb-6">
                <h3 className="text-lg font-semibold text-white mb-3 flex items-center gap-2">
                  <span className="px-2 py-1 bg-gray-500/20 text-gray-400 text-xs rounded">OPTIONAL</span>
                ...
                <div className="space-y-2">
                  {optionalModels.map((model) => {
                    const installed = loraStatus[model.name]?.installed;
                    return (
                      <div key={model.name} className={`border rounded-lg p-3 ${
                        installed 
                          ? 'bg-green-500/5 border-green-500/30' 
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs text-gray-400">{installed ? 'Installed' : 'Not installed'}</span>
                        </div>
                      </div>
                    );
                  })}
