...
export const AnalyticsDashboard: React.FC = () => {
  ...
  const storagePercent = subscription.features.storageLimit !== Infinity
    ? Math.min(stats.storage.used / subscription.features.storageLimit * 100, 100)
    : 0;

  const projectsPercent = subscription.features.maxProjects !== Infinity
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            แพ็กเกจ {subscription.tier.toUpperCase()}
          </p>
        </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-purple-700 mt-1">
                  Upgrade เป็น {nextTier.toUpperCase()} ได้เพียง ฿{nextTierPrice.monthlyPrice.toLocaleString()}/เดือน
                  (ตอนนี้ลด 50%!)
                </p>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          ...
          <div className="mt-4 pt-4 border-t space-y-1 text-sm text-gray-600">
            ...
            <p>• รวมทั้งหมด: {stats.images.totalCreditsUsed + stats.videos.totalCreditsUsed} credits</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className={`text-2xl ${storagePercent > 80 ? 'text-red-500' : 'text-blue-500'}`}>
              {stats.storage.used.toFixed(0)}/{subscription.features.storageLimit === Infinity ? '∞' : subscription.features.storageLimit} MB
            </span>
          </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className={`text-2xl ${projectsPercent > 80 ? 'text-red-500' : 'text-purple-500'}`}>
              {stats.projects.total}/{subscription.features.maxProjects === Infinity ? '∞' : subscription.features.maxProjects}
            </span>
          </div>
          ...
          <div className="mt-4 pt-4 border-t space-y-1 text-sm text-gray-600">
            <p>• ตัวละคร: {stats.characters.total}/{subscription.features.maxCharacters === Infinity ? '∞' : subscription.features.maxCharacters}</p>
            <p>• ฉาก: {stats.scenes.total}/{subscription.features.maxScenes === Infinity ? '∞' : subscription.features.maxScenes}</p>
          </div>
        </div>
      </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-lg font-semibold text-blue-600">
                {stats.images.generated > 0 
                  ? ((stats.images.generated / (stats.images.generated + stats.images.failed)) * 100).toFixed(1)
                  : 0}%
              </span>
            </div>
          </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium">
                      {entry.type === 'image' ? 'สร้างรูปภาพ' : entry.type === 'video' ? 'สร้างวิดีโอ' : 'Text Generation'}
                    </p>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            ...
            <p className="text-3xl font-bold text-green-600">{stats.images.generated + stats.images.failed}</p>
