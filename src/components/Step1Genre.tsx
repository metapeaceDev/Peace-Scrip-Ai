...
const Step1Genre: React.FC<Step1GenreProps> = ({ scriptData, updateScriptData, nextStep, setScriptData, setCurrentStep, onRegisterUndo }) => {
  ...
  // Initialize Default Prompt based on script data
  useEffect(() => {
    if (!posterPrompt && scriptData.title) {
  ...
  }, [scriptData.title, scriptData.mainGenre, posterPrompt]);
  ...
  const handleAutoGenerate = async () => {
    ...
    setError(null);
    try {
      const generatedData = await generateFullScriptOutline(scriptData.title, scriptData.mainGenre, scriptData.secondaryGenres, scriptData.language);
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-cyan-400 mb-6">STEP 1: Genre, story line to be told</h2>
              ...
              <div className="flex flex-col gap-4 items-center md:items-start shrink-0">
                  <div className="w-56 h-[298px] bg-gray-900 rounded-lg shadow-2xl flex items-center justify-center border-2 border-gray-700 overflow-hidden relative group">
                     {scriptData.posterImage ? (
                         <img src={scriptData.posterImage} alt="Movie Poster" className="w-full h-full object-cover transition-transform group-hover:scale-105 duration-700" />
                     ) : (
                         <div className="text-center p-4">
                             <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-gray-700 mx-auto mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="flex-1 flex flex-col justify-between">
                  ...
                  <div className="bg-gray-900/50 border border-gray-700 rounded-lg p-4 flex-1 flex flex-col">
                      <div className="flex justify-between items-center mb-2">
                          <label className="text-xs font-bold text-cyan-500 uppercase tracking-wider">AI Generation Prompt</label>
                          <button onClick={resetPrompt} className="text-[10px] text-gray-500 hover:text-white underline" title="Reset to default based on current title/genre">
                              Auto-Fill Prompt
                          </button>
                      </div>
                      <textarea 
                      ...
                      >
                          {isGeneratingPoster ? (
                              ...
                              <>
                                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 group-hover:scale-110 transition-transform" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V5a2 2 0 00-2-2H4zm12 12H4l4-8 3 6 2-4 3 6z" clipRule="evenodd" /></svg>
      ...
      <div className="space-y-6">
        ...
        <div>
          <label htmlFor="mainGenre" className="block text-sm font-medium text-gray-300 mb-2">1.1 Main story line</label>
          <select
            id="mainGenre"
            name="mainGenre"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label htmlFor="secondaryGenre1" className="block text-sm font-medium text-gray-300 mb-2">1.2 Secondary story line 1</label>
              <select
                id="secondaryGenre1"
            ...
            <div>
              <label htmlFor="secondaryGenre2" className="block text-sm font-medium text-gray-300 mb-2">1.3 Secondary story line 2</label>
              <select
                id="secondaryGenre2"
