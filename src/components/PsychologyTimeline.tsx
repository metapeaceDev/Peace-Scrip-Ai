...
export const PsychologyTimeline: React.FC<PsychologyTimelineProps> = ({ timeline, onClose }) => {
  ...
  const { characterName, snapshots, changes, overallArc } = timeline;
              ...
              {changes.map((change, i) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h4 className="font-bold text-white">
                          Scene {change.sceneNumber}: {change.plotPoint}
                        </h4>
                        <div
                          className={`inline-flex items-center gap-2 mt-1 px-3 py-1 rounded-full text-xs ${
                            change.karmaType === 'กุศลกรรม'
                              ? 'bg-green-500/20 text-green-400 border border-green-500/50'
                              : change.karmaType === 'อกุศลกรรม'
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                      <div className="bg-blue-500/10 border border-blue-500/30 rounded p-3">
                        ...
                        <div className="text-xs text-gray-300 space-y-1">
                          {change.actions.กาย.length > 0 ? (
                            change.actions.กาย.map((action, idx) => (
                      ...
                      <div className="bg-purple-500/10 border border-purple-500/30 rounded p-3">
                        ...
                        <div className="text-xs text-gray-300 space-y-1">
                          {change.actions.ใจ.length > 0 ? (
                            change.actions.ใจ.map((thought, idx) => (
                    ...
                    {(Object.keys(change.consciousnessChanges).length > 0 ||
                      Object.keys(change.defilementChanges).length > 0) && (
                      <div className="mt-3 grid grid-cols-2 gap-3">
                        {Object.keys(change.consciousnessChanges).length > 0 && (
                          <div className="bg-green-500/10 rounded p-2">
                            ...
                            <div className="text-xs text-gray-300 space-y-1">
                              {Object.entries(change.consciousnessChanges).map(
                                ([virtue, delta]) => (
                                  <div key={virtue}>
                        ...
                        {Object.keys(change.defilementChanges).length > 0 && (
                          <div className="bg-red-500/10 rounded p-2">
                            ...
                            <div className="text-xs text-gray-300 space-y-1">
                              {Object.entries(change.defilementChanges).map(
                                ([defilement, delta]) => (
                                  <div key={defilement}>
