...
export const AddAdminModal: React.FC<AddAdminModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleConfirmAdd = async () => {
    ...
    try {
      ...
      if (
        err.message?.includes('มีคำเชิญที่รออยู่แล้ว') ||
            ...
            <div style={{ padding: '1.5rem' }}>
              ...
              >
                <p style={{ margin: 0, fontSize: '0.95rem' }}>
                  <strong>⚠️ โปรดทราบ:</strong>
                  <br />
                  • ระบบจะส่งอีเมลคำเชิญไปยังผู้ใช้
                  <br />
                  • ผู้ใช้ต้องกดยืนยันก่อนจะได้รับสิทธิ์จริง
                  <br />• คำเชิญจะหมดอายุใน 7 วัน
                </p>
              </div>
              ...
              >
                ...
                <div style={{ marginBottom: '0.5rem', color: '#1f2937' }}>
                  ...
                  {role === 'super-admin' ? 'Super Admin' : role === 'admin' ? 'Admin' : 'Viewer'}
                </div>
                <div style={{ color: '#1f2937' }}>
                  <strong style={{ color: '#1f2937' }}>🔐 สิทธิ์:</strong>
                  <ul style={{ marginTop: '0.5rem', marginLeft: '1.5rem', color: '#1f2937' }}>
                    {permissions.canViewAnalytics && (
                      <li style={{ color: '#1f2937' }}>ดู Analytics</li>
                    )}
                    {permissions.canExportData && (
                      <li style={{ color: '#1f2937' }}>Export ข้อมูล</li>
                    )}
                    {permissions.canManageUsers && (
                      <li style={{ color: '#1f2937' }}>จัดการ Admin</li>
                    )}
                    {permissions.canManageSubscriptions && (
                      <li style={{ color: '#1f2937' }}>จัดการ Subscriptions</li>
                    )}
                  </ul>
                </div>
              </div>
          ...
          <form onSubmit={handleSubmit} className="admin-form">
            ...
            >
              <p style={{ margin: 0, lineHeight: '1.6' }}>
                ...
                <br />
                1. กรอกข้อมูลและส่งคำเชิญ
                <br />
                2. ระบบส่งอีเมลไปยังผู้ใช้
                <br />
                3. ผู้ใช้กดยืนยันในอีเมล
                <br />
              ...
              <div className="permissions-grid">
                ...
                <label className="permission-item">
                  <input
                    ...
                    onChange={e =>
                      handlePermissionChange('canManageSubscriptions', e.target.checked)
                    }
                    disabled={loading}
                  />
