...
export type EquipmentType =
  | 'Steadicam' | 'Dolly' | 'Crane' | 'Handheld' | 'Gimbal' | 'Tripod';
...
const CAMERA_MOVEMENT_MAP: Record<CameraMovementType, (equipment?: string, shotSize?: string) => string> = {
...
export function buildCameraMovementContext(shotData: ShotData): string {
  ...
  const movementDescription = CAMERA_MOVEMENT_MAP[movementType]?.(equipment, shotSize) 
...
export function buildEnvironmentalMotionContext(
  ...
  const backgroundMotion = LOCATION_TO_MOTION[locationKey] 
    || 'natural environmental movement, ambient life';
  ...
  const situationText = situations.map(s => s.description).join(' ').toLowerCase();
 ...
 */
export function getMotionModuleStrength(
  shotData: ShotData,
  character: Character
): number {
