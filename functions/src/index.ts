...
export const initializeSuperAdmin = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      'internal',
      error instanceof Error ? error.message : 'Failed to initialize super-admin'
    );
  }
});
 ...
 */
export const createAdminInvitation = functions.https.onCall(async (data, context) => {
  // ตรวจสอบ authentication
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'Must be authenticated to create admin invitation'
    );
  }
  ...
  if (!isAdmin || adminRole !== 'super-admin') {
    ...
    throw new functions.https.HttpsError(
      'permission-denied',
      `Only super-admins can create admin invitations. Your role: ${adminRole || 'none'}`
    );
  }
  ...
  try {
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        'not-found',
        `User with email ${email} not found. User must have an account first.`
      );
    }
    ...
    try {
      ...
      console.warn(
        '⚠️ Email notification failed (continuing with in-app notification):',
        emailError
      );
    }
    ...
    throw new functions.https.HttpsError(
      'internal',
      error instanceof Error ? error.message : 'Failed to create admin invitation'
    );
  }
});
 ...
 */
export const confirmAdminInvitation = functions.https.onCall(async (data, context) => {
  ...
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'Must be authenticated to confirm invitation'
    );
  }
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      'internal',
      error instanceof Error ? error.message : 'Failed to confirm admin invitation'
    );
  }
});
 ...
 */
export const cancelAdminInvitation = functions.https.onCall(async (data, context) => {
  // ตรวจสอบ authentication
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'Must be authenticated to cancel invitation'
    );
  }
  ...
  if (!isAdmin || adminRole !== 'super-admin') {
    throw new functions.https.HttpsError(
      'permission-denied',
      `Only super-admins can cancel admin invitations. Your role: ${adminRole || 'none'}`
    );
  }
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      'internal',
      error instanceof Error ? error.message : 'Failed to cancel admin invitation'
    );
  }
});
...
export const grantAdminAccess = functions.https.onCall(async (data, context) => {
  ...
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'Must be authenticated to grant admin access'
    );
  }
  ...
  if (!isAdmin || adminRole !== 'super-admin') {
    ...
    throw new functions.https.HttpsError(
      'permission-denied',
      `Only super-admins can grant admin access. Your role: ${adminRole || 'none'}`
    );
  }
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      'internal',
      `Failed to grant admin access: ${errorMessage}`
    );
  }
});
 ...
 * เฉพาะ super-admin เท่านั้นที่เรียกใช้ได้
 */
export const revokeAdminAccess = functions.https.onCall(async (data, context) => {
  // ตรวจสอบ authentication
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'Must be authenticated to revoke admin access'
    );
  }
...
export const updateAdminPermissions = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      'internal',
      error instanceof Error ? error.message : 'Failed to update admin permissions'
    );
  }
});
