...
const Studio: React.FC<StudioProps> = ({ projects, onCreateProject, onOpenProject, onDeleteProject, onImportProject, onExportProject }) => {
      ...
      <div className="max-w-7xl mx-auto">
        <header className="flex flex-col md:flex-row justify-between items-center mb-8 border-b border-gray-700 pb-6">
            ...
            <div className="flex gap-3">
                ...
                <input type="file" ref={fileInputRef} onChange={handleFileChange} accept=".json,.txt,.docx,.pdf" className="hidden" />
                ...
                {projects.sort((a,b) => b.lastModified - a.lastModified).map(project => (
                    <div key={project.id} className="bg-gray-800 rounded-xl border border-gray-700 hover:border-cyan-500/50 transition-all hover:shadow-xl hover:shadow-cyan-900/10 group flex flex-col h-[280px] relative overflow-hidden">
                         ...
                         <div className="absolute top-0 right-0 p-2 opacity-0 group-hover:opacity-100 transition-opacity z-20 flex gap-1">
                             ...
                             >
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd" /></svg>
                                {deleteConfirmationId === project.id && <span className="text-[10px] font-bold whitespace-nowrap">CONFIRM</span>}
                             </button>
                         </div>

                         <div className="flex-1 cursor-pointer z-10 flex flex-col p-4" onClick={() => onOpenProject(project.id)}>
                             <div className="flex items-start justify-between mb-auto">
                                <span className="inline-block px-2 py-0.5 rounded-full text-[10px] font-bold bg-cyan-900/80 text-cyan-400 border border-cyan-500/30 backdrop-blur-sm shadow-sm">
                                    {PROJECT_TYPES.find(t => t.value === project.type)?.label.split('(')[0] || project.type}
                                </span>
                             </div>
