...
### 3. ระบบติดตามต้นทุนแบบละเอียดและแจ้งเตือนงบประมาณ (Granular Tracking & Budget Alerts) ✅

**ปัญหาเดิม**: การคำนวณต้นทุนใช้การประมาณจากจำนวนตัวอักษร ซึ่งไม่แม่นยำสำหรับโมเดล Gemini 2.5 Flash และไม่มีระบบแจ้งเตือนเมื่องบประมาณเกินกำหนด

**การแก้ไข**:

- ✅ **Granular Token Tracking**:
  - ใช้ `countTokens` API ของ Gemini เพื่อรับจำนวน Token ที่แท้จริง (Input/Output)
  - อัปเดต `geminiService.ts` ให้นับ Token ก่อนและหลังการ generate
  - บันทึกข้อมูล `tokens` ลงใน metadata ของ Firestore collection `generations`
- ✅ **Pricing Model Update**:
  - อัปเดตราคาใน `types/analytics.ts` เป็นแบบ Token-based (Input ~$0.075/1M, Output ~$0.30/1M)
- ✅ **Budget Alert System**:
  - เพิ่มฟังก์ชัน `checkGlobalDailyBudget` ใน `modelUsageTracker.ts`
  - ตั้งค่าแจ้งเตือนเมื่อค่าใช้จ่ายรายวันเกิน **฿500**
  - สร้างเอกสารแจ้งเตือนใน collection `system_alerts`
- ✅ **Type Safety**:
  - อัปเดต Interface `GenerationDetails` ให้รองรับข้อมูล `tokens`
  - แก้ไข Build Error ที่เกิดจากการเปลี่ยนแปลง Type

**ไฟล์ที่เปลี่ยนแปลง**:

- `src/services/geminiService.ts` (เพิ่ม countTokens logic)
- `src/services/modelUsageTracker.ts` (เพิ่ม budget check logic)
- `src/types/analytics.ts` (อัปเดตราคาและ Type definitions)

### 2. การแก้ไขความถูกต้องของข้อมูล (Data Consistency) ✅

**ปัญหาเดิม**: ข้อมูลใน Dashboard ไม่ตรงกัน (Audio duration เป็น 0, Storage size เป็น 0) และมี Type mismatch

**การแก้ไข**:

- ✅ แก้ไข `audioService.ts` ให้ส่ง `duration` ที่ถูกต้อง
- ✅ แก้ไข `storageService.ts` ให้คำนวณ `size` ของไฟล์ที่อัปโหลด
- ✅ ปรับปรุง Type definitions ใน `types.ts` และ `analytics.ts`
